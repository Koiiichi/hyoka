...
|---|---|---|---|---|---|
| `threat-hunt-windows-eventlogs` | empire_mimikatz_logonpasswords | 37 (C2 beacons + lsass reads) | 0% | 0.015,0.60,0.015,0.015,0.015 | over-flag / partial |
| `lateral-movement-psexec-hunt` | empire_psexec_dcerpc_tcp_svcctl | 4 (cross-source: 4624+4697+7045+Sysmon1) | 0% | 0.007,0.006,0.30,0.24,0.44 | precision collapse (found 4/4, flagged 1191) / partial cross-source |
| `persistence-run-key-hunt` | empire ...run_keys_elevated_user | 11 (run-key + C2 beacons) | 0% | 0.069 x5 | precision collapse (found 11/11, flagged 309) |
| `discovery-account-enum-hunt` | empire net_localgroup + net_local_users | 14 (net.exe recon + 4798/4799) | 0% | 0.020,0.020,0.023,0.020,0.024 | cross-source recall gap (5/14) + over-flag |
| `execution-vbs-launcher-hunt` | empire_launcher_vbs | 5 (script-host process tree + C2 network) | pending | oracle 1.0; lazy top flag-all-process 0.53 | (pilot) |
| `defense-evasion-injection-hunt` | covenant_lolbin_wuauclt_createremotethread | 3 (Sysmon 1 LOLBin + 7 non-Windows DLL + 8 CreateRemoteThread) | pending | oracle 1.0; lazy top CRT-only 0.50 | (pilot) |
| `privesc-uac-bypass-hunt` | empire_uac_shellapi_fodhelper | 7 (ms-settings\Run-command registry hijack + fodhelper + child) | pending | oracle 1.0; lazy top flag-all-process 0.14 | (pilot) |
