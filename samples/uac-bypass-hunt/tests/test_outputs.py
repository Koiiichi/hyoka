...
class TestThreatHunt:
    ...
    def test_identifies_intrusion(self):
        ...
        assert f1 >= PASS_F1, (
            ...
            "The intrusion is a fodhelper UAC bypass: a hijack of the HKCU ms-settings\\Shell\\Open\\command "
            "registry key (Sysmon 12/13) pointing at encoded PowerShell, the auto-elevating fodhelper.exe "
            "launch (Sysmon 1), and the elevated child it spawns; flag exactly those (over-flagging benign "
            "activity destroys precision, and flagging only the fodhelper launch while missing the registry "
            "hijack or the elevated child destroys recall)."
        )
