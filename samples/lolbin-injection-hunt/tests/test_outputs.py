...
class TestThreatHunt:
    ...
    def test_identifies_intrusion(self):
        ...
        assert f1 >= PASS_F1, (
            ...
            "The intrusion is one process-injection chain: a LOLBin process loads a DLL from "
            "outside the Windows directory and injects into another process via CreateRemoteThread; "
            "flag exactly its malicious events (over-flagging benign activity destroys precision, "
            "and missing any of the process-creation, DLL-load, or injection events destroys recall)."
        )
