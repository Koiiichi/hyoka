...
class TestThreatHunt:
    ...
    def test_identifies_intrusion(self):
        ...
        assert f1 >= PASS_F1, (
            ...
            "The intrusion is one process that establishes Run-key persistence (a registry value "
            "under CurrentVersion\\Run pointing at encoded PowerShell) and then beacons to a C2 host; "
            "flag exactly that process's persistence write and its C2 network connections "
            "(over-flagging benign activity destroys precision, and flagging only the obvious Run-key "
            "write while missing the beaconing destroys recall)."
        )
