...
class TestThreatHunt:
    ...
    def test_identifies_intrusion(self):
        ...
        assert f1 >= PASS_F1, (
            ...
            "The intrusion is a cross-source account/group discovery campaign: net.exe/net1.exe "
            "reconnaissance commands (Sysmon) plus the Windows Security membership-enumeration audit "
            "events (4798/4799) that record the same behaviour in a different log; flag exactly those "
            "(over-flagging benign activity destroys precision, and catching the activity in only one "
            "log source destroys recall)."
        )
